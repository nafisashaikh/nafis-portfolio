...
export default function ImpactStrip() {
  ...
  return (
    <section id="impact" className="py-16 max-w-6xl mx-auto px-6 text-white scroll-mt-20">
      ...
      <div className="absolute left-1/4 h-72 w-72 bg-theme-600/5 rounded-full blur-3xl pointer-events-none" />

      {/* Header heading */}
      <div className="flex flex-col gap-2 mb-10">
        <span className="font-mono text-xs tracking-[0.25em] text-theme-500 uppercase font-semibold">Top 3 Accomplishments</span>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {activePersona.achievements.map((item, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-white/[0.02] border border-white/10 p-6 relative overflow-hidden transition-all duration-300 hover:border-theme-500/30 group shadow-md"
            >
              {/* Corner accent decal */}
              <div className="absolute top-0 right-0 w-8 h-8 opacity-40 group-hover:opacity-100 transition-opacity">
                <div className="absolute top-2.5 right-2.5 w-1.5 h-1.5 bg-theme-500" />
              </div>
              ...
              <h3 className="text-sm font-mono font-bold uppercase tracking-wide text-white group-hover:text-theme-400 transition-colors mb-2">
                {item.title}
              </h3>
